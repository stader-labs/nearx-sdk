...
export const getTotalNearxSupply = async (network: Network, accountId: string) => {
  const nearxPoolClient = await NearxPoolClient.new(network, accountId);

  return nearxPoolClient.getTotalNearxSupply();
};

export const getTotalNearStaked = async (network: Network, accountId: string) => {
  const nearxPoolClient = await NearxPoolClient.new(network, accountId);

  return nearxPoolClient.getTotalNearStaked();
};

export const getUserNearxBalance = async (network: Network, accountId: string) => {
  const nearxPoolClient = await NearxPoolClient.new(network, accountId);
